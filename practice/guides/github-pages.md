...
## 3. Как включить Pages для Actions (без картинок)

Если Pages ещё не включены, шаг `actions/configure-pages` будет падать с ошибкой 404. Это означает, что GitHub Pages не активирован для репозитория. Чтобы включить:

1. Откройте репозиторий на GitHub.
2. Перейдите в `Settings → Pages`.
3. В блоке **Build and deployment** найдите поле **Source**.
4. Выберите **GitHub Actions** и нажмите **Save**.

После сохранения Pages будет включён, и workflow сможет получать `baseURL` и деплоить артефакт.

Важно: запуск workflow всё равно зависит от `on:` в `.github/workflows/hugo.yaml` (обычно `push` в `main` или ручной запуск).

## 4. Почему GitHub Actions — базовый путь для Hugo
...
## 5. Базовый workflow для Hugo Pages
...
## 6. Кэш изображений и экономия времени
...
## 8. SSH‑ключ и агент для публикации (WSL)
...
## 9. Если доступ к репозиторию с другого аккаунта
