# Git, SSH, ssh-agent и GitHub: большой практический учебник

Этот учебник объясняет полный рабочий цикл: от установки Git и настройки имени до безопасной работы через SSH‑ключи и автоматизации без постоянного ввода пароля. Текст написан в практическом стиле и рассчитан на студентов.
...
Git работает локально и не требует интернета для большинства операций. GitHub — это удалённый хостинг для репозиториев.
...
Укажите имя и email для коммитов (локально в проекте или глобально для всех репозиториев).
...
GitHub работает поверх Git и не заменяет его, а дополняет.

## 8. Что такое SSH и зачем он нужен

SSH — это безопасный протокол для доступа к удалённым серверам. В Git‑контексте он используется, чтобы подключаться к GitHub без пароля (через ключ).
...
Если ключ защищён паролем, его можно загрузить в `ssh-agent`. Тогда Git будет использовать ключ без повторного ввода.
...
Если нужно использовать разные аккаунты GitHub, удобно завести алиасы в `~/.ssh/config`:
